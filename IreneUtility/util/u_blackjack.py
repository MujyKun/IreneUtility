...
class BlackJack(Base):
    ...
    async def check_in_game(self, user_id, ctx):  # this is meant for when it is accessed by commands outside of BlackJack.
        ...
        check = self.ex.first_result(await self.ex.conn.fetchrow("SELECT COUNT(*) From blackjack.games WHERE player1 = $1 OR player2 = $1", user_id))
        if check:
            await ctx.send(f"> **{ctx.author}, you are already in a pending/active game. Please type {await self.ex.get_server_prefix(ctx)}endgame.**")
            return True

    async def add_bj_game(self, user_id, bid, ctx, mode):
        """Add the user to a blackjack game."""
        await self.ex.conn.execute("INSERT INTO blackjack.games (player1, bid1, channelid) VALUES ($1, $2, $3)", user_id, str(bid), ctx.channel.id)
        game_id = await self.get_game_by_player(user_id)
        if mode != "bot":
            await ctx.send(f"> **There are currently 1/2 members signed up for BlackJack. To join the game, please type {await self.ex.get_server_prefix(ctx)}joingame {game_id} (bid)** ")
    ...
    async def process_bj_game(self, ctx, amount, user_id):
        ...
        if amount >= 0:
            if not await self.check_in_game(user_id, ctx):
                if amount > await self.ex.u_currency.get_balance(user_id):
    ...
    async def get_game_by_player(self, player_id):
        ...
        return self.ex.first_result(await self.ex.conn.fetchrow("SELECT gameid FROM blackjack.games WHERE player1 = $1 OR player2 = $1", player_id))

    async def get_game(self, game_id):
        """Get the game from its ID"""
        return await self.ex.conn.fetchrow("SELECT gameid, player1, player2, bid1, bid2, channelid FROM blackjack.games WHERE gameid = $1", game_id)

    async def add_player_two(self, game_id, user_id, bid):
        """Add a second player to a blackjack game."""
        await self.ex.conn.execute("UPDATE blackjack.games SET player2 = $1, bid2 = $2 WHERE gameid = $3 ", user_id, str(bid), game_id)

    async def get_current_cards(self, user_id):
        """Get the current cards of a user."""
        in_hand = self.ex.first_result(await self.ex.conn.fetchrow("SELECT inhand FROM blackjack.currentstatus WHERE userid = $1", user_id))
        if in_hand is None:
            return []
    ...
    async def check_player_standing(self, user_id):
        ...
        return self.ex.first_result(await self.ex.conn.fetchrow("SELECT stand FROM blackjack.currentstatus WHERE userid = $1", user_id)) == 1

    async def set_player_stand(self, user_id):
        """Set a player to stand."""
        await self.ex.conn.execute("UPDATE blackjack.currentstatus SET stand = $1 WHERE userid = $2", 1, user_id)

    async def delete_player_status(self, user_id):
        """Remove a player's status from a game."""
        await self.ex.conn.execute("DELETE FROM blackjack.currentstatus WHERE userid = $1", user_id)

    async def add_player_status(self, user_id):
        """Add a player's status to a game."""
        await self.delete_player_status(user_id)
        await self.ex.conn.execute("INSERT INTO blackjack.currentstatus (userid, stand, total) VALUES ($1, $2, $2)", user_id, 0)

    async def get_player_total(self, user_id):
        """Get a player's total score."""
        return self.ex.first_result(await self.ex.conn.fetchrow("SELECT total FROM blackjack.currentstatus WHERE userid = $1", user_id))

    async def get_card_value(self, card):
        """Get the value of a card."""
        return self.ex.first_result(await self.ex.conn.fetchrow("SELECT value FROM blackjack.cards WHERE id = $1", card))

    async def get_all_cards(self):
        """Get all the cards from a deck."""
        card_tuple = await self.ex.conn.fetch("SELECT id FROM blackjack.cards")
        all_cards = []
    ...
    async def get_card_name(self, card_id):
        ...
        return self.ex.first_result(await self.ex.conn.fetchrow("SELECT name FROM blackjack.cards WHERE id = $1", card_id))
    ...
    async def set_aces_used(self, card_list, user_id):
        ...
        await self.ex.conn.execute("UPDATE blackjack.currentstatus SET acesused = $1 WHERE userid = $2", cards, user_id)

    async def get_aces_used(self, user_id):
        """Get the aces that were changed from 11 to 1."""
        aces_used = self.ex.first_result(await self.ex.conn.fetchrow("SELECT acesused FROM blackjack.currentstatus WHERE userid = $1", user_id))
    ...
    def check_if_bot(self, user_id):
        ...
        return str(self.get_int_index(self.ex.keys.bot_id, 9)) in str(user_id)
    ...
    async def add_card(self, user_id):
        ...
        game_id = await self.get_game_by_player(user_id)
        game = await self.get_game(game_id)
        channel = await self.ex.client.fetch_channel(game[5])
        ...
        if not stand:
            ...
            if current_total + random_card_value > 21:
                ...
                current_total = current_total + random_card_value
            await self.ex.conn.execute("UPDATE blackjack.currentstatus SET inhand = $1, total = $2 WHERE userid = $3", cards, current_total, user_id)
            if current_total > 21:
    ...
    async def announce_winner(self, channel, winner, loser, winner_points, loser_points, win_amount):
        ...
        if self.check_if_bot(winner):
            await channel.send(f"> **<@{self.ex.keys.bot_id}> has won ${int(win_amount):,} (CURRENCY DISABLED) with {winner_points} points against <@{loser}> with {loser_points}.**")
        elif self.check_if_bot(loser):
            await channel.send(f"> **<@{winner}> has won ${int(win_amount):,} (CURRENCY DISABLED) with {winner_points} points against <@{self.ex.keys.bot_id}> with {loser_points}.**")
        else:
    ...
    async def announce_tie(self, channel, player1, player2, tied_points):
        ...
        if self.check_if_bot(player1) or self.check_if_bot(player2):
            await channel.send(f"> **<@{player1}> and <@{self.ex.keys.bot_id}> have tied with {tied_points}**")
        else:
    ...
    async def finish_game(self, game_id, channel):
        ...
        if player2_score < 12 and self.check_if_bot(game[2]):
            ...
            # player1_current_bal = await self.ex.u_currency.get_balance(game[1])
            # player2_current_bal = await self.ex.u_currency.get_balance(game[2])
            player1_current_bal = 0
            player2_current_bal = 0
            if winner == 'player1':
                # await self.ex.u_currency.update_balance(game[1], player1_current_bal + int(game[4]))
                if not self.check_if_bot(game[2]):
                    pass
                    # await self.ex.u_currency.update_balance(game[2], player2_current_bal - int(game[4]))
                await self.announce_winner(channel, game[1], game[2], player1_score, player2_score, game[4])
            elif winner == 'player2':
                if not self.check_if_bot(game[2]):
                    pass
                    # await self.ex.u_currency.update_balance(game[2], player2_current_bal + int(game[3]))
                # await self.ex.u_currency.update_balance(game[1], player1_current_bal - int(game[3]))
    ...
    async def delete_game(self, game_id):
        ...
        game = await self.get_game(game_id)
        await self.ex.conn.execute("DELETE FROM blackjack.games WHERE gameid = $1", game_id)
        await self.ex.conn.execute("DELETE FROM blackjack.currentstatus WHERE userid = $1", game[1])
        await self.ex.conn.execute("DELETE FROM blackjack.currentstatus WHERE userid = $1", game[2])
        log.console(f"Game {game_id} deleted.")

    async def delete_all_games(self):
        """Delete all blackjack games."""
        all_games = await self.ex.conn.fetch("SELECT gameid FROM blackjack.games")

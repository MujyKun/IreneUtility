...
class Album:
    def __init__(self, album_name, idol_cards,
        ...
        if not album_active_time:
            ...
            album_active_time = datetime.timedelta(hours=base_util.ex.u_objects.GachaValues.album_active_time_in_hours)
    ...
    async def create_album(album_name, idol_cards):
        popularity = await base_util.ex.u_gacha.random_album_popularity()
    ...
    async def calculate_income_rate(self):
        max_income_rate = base_util.ex.u_objects.GachaValues.album_max_income_rate
    ...
    async def skill_completion_multiplier(self):
        ...
        if number_of_non_zero_skills == 3:
            return base_util.ex.u_objects.GachaValues.album_three_skill_mult
        elif number_of_non_zero_skills == 2:
            return base_util.ex.u_objects.GachaValues.album_two_skill_mult
        else:
            return base_util.ex.u_objects.GachaValues.album_one_skill_mult
